...
class Command(BaseCommand):
    help = 'Fetch housing regions from ss.com and sync to DB'

    def handle(self, *args, **options):
        all_regions = self._fetch_all_regions()
        self.stdout.write(
            f'Fetched {len(all_regions)} housing regions from ss.com'
        )
    ...
    def _find_direct_children(soup, parent_path):
        ...
        other one-level-deep links on the same page (e.g. /search/, /new/)
        don't carry this class and must be excluded.
        """
